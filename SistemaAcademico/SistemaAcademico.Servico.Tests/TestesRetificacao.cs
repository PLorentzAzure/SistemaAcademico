...
namespace SistemaAcademico.Servico.Tests
{
    ...
    public class TestesRetificacao : TesteWebApi
    {
        ...
        public void FluxoCompleto()
        {
            ...
            var rfCriacao = new RetificacaoFalta { IdMatricula = 1, IdOferta = 2, DataFalta = DateTime.Now.Date.AddDays(-1), Justificativa = "Tava chuvendo." };

            var rfCriada = RequisicaoWeb.Requisicao(url, method: "POST", postData: rfCriacao).Deserializa<RetificacaoFalta>();
            ...
            Assert.IsTrue(rfCriada.Status == StatusServico.Pendente);
            Assert.IsTrue(rfCriada.DataRequisicao > rfCriacao.DataFalta);

            RequisicaoWeb.Requisicao($"{url}/{rfCriacao.Id}/Aprovar", method: "PUT");
            var rfAprovada = RequisicaoWeb.Requisicao($"{url}/{rfCriacao.Id}").Deserializa<RetificacaoFalta>();

            Assert.IsTrue(comparador.Equals(rfCriada, rfAprovada));
            Assert.IsTrue(rfCriada.Status == StatusServico.Aprovado);
        }
